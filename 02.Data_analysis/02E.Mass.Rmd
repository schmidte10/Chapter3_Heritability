...
MASS.x ~ MASS_(FE)MALE*TEMPERATURE + (1|CLUTCH_NUMBER)
```
...
```{r eval=FALSE, echo=TRUE}
MASS.x ~ MASS_(FE)MALE*TEMPERATURE:PARENTAL_DAYS_IN_TREATMENT + (1|CLUTCH_NUMBER)
```
...
##### Main hypothesis
```{r}
model1a <- glmmTMB(MASS.x ~ MASS_MALE*TEMPERATURE + (1|CLUTCH_NUMBER), 
                    family='gaussian', 
                    data=m1_df)
```
...
```{r}
model1b <- glmmTMB(MASS.x ~ MASS_MALE*TEMPERATURE*as.numeric(PARENTAL_DAYS_IN_TREATMENT) + (1|CLUTCH_NUMBER), 
                    family='gaussian', 
                    data=m1_df)
```
...
##### Main hypothesis
```{r}
model1a <- glmmTMB(MASS.x ~ MASS_FEMALE*TEMPERATURE + (1|CLUTCH_NUMBER), 
                    family='gaussian', 
                    data=m1_df)
```
...
##### Alternative hypothesis 
```{r}
model1b <- glmmTMB(MASS.x ~ MASS_FEMALE*TEMPERATURE*as.numeric(PARENTAL_DAYS_IN_TREATMENT) + (1|CLUTCH_NUMBER), 
                    family='gaussian', 
                    data=m1_df)
```
...
##### Main hypothesis
```{r}
model1a <- glmmTMB(MASS.x ~ MASS_MIDPOINT*TEMPERATURE + (1|CLUTCH_NUMBER), 
                    family=gaussian(), 
                    data=m1_df)
```
...
##### Alternative hypothesis 
```{r}
model1b <- glmmTMB(MASS.x ~ MASS_MIDPOINT*TEMPERATURE*as.numeric(PARENTAL_DAYS_IN_TREATMENT) + (1|CLUTCH_NUMBER), 
                    family=gaussian(), 
                    data=m1_df)
```
