...
```{r}
model1a <- glmmTMB(STANDARD_LENGTH ~ scale(SL_MALE, center=TRUE)*TEMPERATURE + scale(DENSITY, center=TRUE) + scale(AGE_DAYS, center=FALSE) + (1| MALE),                     
                   family=gaussian(), 
                   data=m3_df_all)
```
...
```{r}
model1b <- glmmTMB(STANDARD_LENGTH ~ scale(SL_MALE, center=TRUE)*TEMPERATURE + scale(as.numeric(PARENTAL_DAYS_IN_TREATMENT), center=TRUE) +                     scale(DENSITY, center=TRUE) +
...
m3.plot <- ggplot(data = m3.sl.df, aes(x=SL_MALE, y=emmean)) + 
  ...
  scale_y_continuous(limits = c(20,30), breaks = seq(20,30,by=2))+
