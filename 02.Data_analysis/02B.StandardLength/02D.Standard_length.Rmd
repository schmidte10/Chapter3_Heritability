...
model1a.blup <- glmmTMB(STANDARD_LENGTH ~ 1 + as.numeric(AGE)*TEMPERATURE + scale(DENSITY, scale=TRUE, center=TRUE) + (1|fSL_MALE_GROUP), 
                    family=gaussian(), 
                    data=df.blup) 
...
```{r}
model2a.blup <- glmmTMB(STANDARD_LENGTH ~ 1 + poly(as.numeric(AGE, 2, raw=TRUE))*TEMPERATURE + scale(DENSITY, scale=TRUE, center=TRUE) + (1|fSL_MALE_GROUP), 
                    family=gaussian(), 
                    data=df.blup) 
...
```{r}
model3a.blup <- glmmTMB(STANDARD_LENGTH ~ 1 + poly(as.numeric(AGE, 2, raw=TRUE))*TEMPERATURE + scale(DENSITY, scale=TRUE, center=TRUE) + (1+AGE|fSL_MALE_GROUP), 
                    family=gaussian(), 
                    data=df.blup) 
...
```{r}
genotype_blups <- ranef(model3a.blup)$fSL_MALE_GROUP
