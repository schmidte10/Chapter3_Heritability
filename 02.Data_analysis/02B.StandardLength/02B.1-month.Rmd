---
title: "02B.Standard Length: 1-month"
...
```{r}
model1b <- glmmTMB(STANDARD_LENGTH ~ scale(SL_MALE, center = TRUE)*TEMPERATURE + scale(as.numeric(PARENTAL_DAYS_IN_TREATMENT), center = TRUE) + scale(DENSITY, center = TRUE) +(1| MALE),
                   family=gaussian(), 
                   data=m1_df_all)
```
...
m1.sl <- emmeans(model1b, ~ SL_MALE*TEMPERATURE, 
...
mod = lm(mean.sl ~ mean.sl.male*TEMPERATURE, 
         data =m1.sl.obs.summarize)
mod.sl <- emmeans(mod, ~ mean.sl.male*TEMPERATURE, 
                 at =list(mean.sl.male=seq(from =min(m1.sl.obs.summarize$mean.sl.male), to =max(m1.sl.obs.summarize$mean.sl.male), by=.25))) |> as.data.frame()

m1.plot <- ggplot(data = m1.sl.df, aes(x=SL_MALE, y=emmean)) + 
  stat_smooth(data=m1.sl.obs.summarize, aes(x =mean.sl.male, 
...
m1.sl.obs.summarize2 <- m1.sl.obs.summarize |> 
  group_by(TEMPERATURE) |> 
  mutate(temp_mean =mean(mean.sl), 
         temp_sd =sd(mean.sl)) |> 
  distinct(temp_mean, .keep_all = TRUE)

m1.plot2 <- ggplot(m1.sl.obs.summarize, aes(x=TEMPERATURE, y=mean.sl, color=TEMPERATURE)) + 
  geom_pointrange(data=m1.sl.obs.summarize2, 
                  aes(x=TEMPERATURE, 
                      y=temp_mean, 
...
m1.density <- ggdensity(m1.sl.obs, x="Fit", 
  ...
  scale_x_continuous(limits = c(8,18), breaks = seq(8,18,by=2))+
          clean_theme() + rotate() + 
