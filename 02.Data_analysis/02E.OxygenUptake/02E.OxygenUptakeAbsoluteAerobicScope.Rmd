...
```{r}
model1a <- glmmTMB(AAS ~ scale(as.numeric(AAS.x), center=TRUE)*Temperature + scale(Mass, center=TRUE) + (1| Male),    
                   family=gaussian(),
                   data=df)
```
...
df <- df |> drop_na(AAS, AAS.x, Mass)
m3.aas <- emmeans(model1a, ~ AAS.x*Temperature, 
                 at =list(AAS.x=seq(from =min(df$AAS.x), to =max(df$AAS.x), by=.1)))
