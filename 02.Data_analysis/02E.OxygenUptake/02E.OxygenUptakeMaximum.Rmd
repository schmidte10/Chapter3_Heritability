...
```{r}
model1a <- glmmTMB(Max ~ scale(as.numeric(Max_MALE), center=TRUE)*Temperature + scale(Mass, center=TRUE) + (1| Male),    
                   family=gaussian(),
                   data=df)
```
...
df <- df |> drop_na(Max, Max_MALE, Mass)
m3.max <- emmeans(model1a, ~ Max_MALE*Temperature, 
                 at =list(Max_MALE=seq(from =min(df$Max_MALE), to =max(df$Max_MALE), by=.1)))
