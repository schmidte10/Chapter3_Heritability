...
df_adults_cleaned2 <- df_males |> 
  ...
  mutate(Temperature.x = coalesce(Temperature.x, Temperature.y), 
         ...
         Resting_kg.x =coalesce(Resting_kg.x, Resting_kg.y)) |>  
  drop_na(Resting.x)
```
...
```{r}
model1a <- glmmTMB(Resting_kg_wet ~ scale(as.numeric(Resting_MALE), center=TRUE)*Temperature + (1| Male) + (1|ChbSys),    
                   family=gaussian(),
                   data=df)
```
...
model1a |> ggemmeans(~Resting_MALE|Temperature) |> 
  plot(add.data =TRUE) 
...
model1a |> emmeans(~Temperature) |>pairs() |> summary()
```

# Summary figure

```{r fig.height = 8, fig.width=12, warning=FALSE} 
df <- df |> drop_na(Resting_kg_wet, Resting_MALE, Mass)
m3.resting <- emmeans(model1a, ~ Resting_MALE*Temperature, 
                 at =list(Resting_MALE=seq(from =min(df$Resting_MALE), to =max(df$Resting_MALE), by=50)))
...
m3.resting.obs.summarize <-  m3.resting.obs |> 
  ...
  summarise(mean.resting =mean(Fit, na.rm=TRUE), 
            mean.resting.male =mean(Resting_MALE, na.rm = TRUE), 
...
m3.resting.obs.summarize2 <- m3.resting.obs.summarize |> 
  group_by(Temperature) |> 
  mutate(temp_mean =mean(mean.resting), 
         temp_sd =sd(mean.resting)) |> 
  distinct(temp_mean, .keep_all = TRUE) 

mod = lm(mean.resting ~ mean.resting.male*Temperature,  
         data =m3.resting.obs.summarize)

mod.resting <- emmeans(mod, ~ mean.resting.male*Temperature, 
                 at =list(mean.resting.male=seq(from =min(m3.resting.obs.summarize$mean.resting.male), to =max(m3.resting.obs.summarize$mean.resting.male), by=10))) |> as.data.frame()

m3.plot <- ggplot(data = m3.resting.df, aes(x=resting_MALE, y=response)) + 
  stat_smooth(data=m3.resting.obs.summarize, aes(x =mean.resting.male, 
                                                  y =mean.resting, color=Temperature), 
              method = "lm", 
              se=FALSE) + 
  geom_ribbon(data=mod.resting, aes(x =mean.resting.male, 
                                                  y =emmean, 
  ...
  xlab("PARENTAL RESTING OXYGEN UPTAKE (mgO h-1 kg-1)") + 
  ylab("OFFSPRING RESTING OXYGEN UPTAKE (mgO h-1 kg-1)") + 
  ggtitle("Offspring-male relationship") +
  theme_classic()+theme(legend.position = 'none')

m3.plot2 <- ggplot(m3.resting.obs.summarize, aes(x=Temperature, y=mean.resting, color=Temperature)) + 
  geom_pointrange(data=m3.resting.obs.summarize2, 
                  aes(x=Temperature, 
                      y=temp_mean, 
