...
model1c.p2 <- glmmTMB(STANDARD_LENGTH ~ poly(as.numeric(AGE_DAYS),2, raw=TRUE)*TEMPERATURE + scale(DENSITY, center=TRUE)  + scale(as.numeric(PARENTAL_DAYS_IN_TREATMENT), center=TRUE) + (1| MALE),                 
                   family=gaussian(), 
                   dispformula = ~AGE,
                   data=growth_data)
```
...
```{r}
AIC(model1a, model1a.p2,model1c.p2, k=6)
AIC(model1a.p2, model1b.p2,model1c.p2,  k=6) 
``` 
...
cv_among <- cv_within |> 
  ...
  mutate(mean_among =mean(CV), 
         sd_among =sd(CV)) |> 
  mutate(cv_among =(sd_among/mean_among)*100) |> 
  ungroup() |> 
  distinct(cv_among, .keep_all = T) |> 
  arrange(TEMPERATURE) |> 
  select(c("TEMPERATURE","AGE","cv_among","mean_among","sd_among"))
cv_among
```
