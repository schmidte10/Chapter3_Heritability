...
```{r}
model1a.p2 |> emtrends(var ="AGE_DAYS", type="response") |> pairs()|> summary(adjust="tukey")
```

# Summary figure 

```{r}
growth <- emmeans(model1a.p2, ~ AGE_DAYS*TEMPERATURE, 
                 at =list(AGE_DAYS=seq(from =min(growth_data$AGE_DAYS), to =max(growth_data$AGE_DAYS), by=.5)))

growth.df <- as.data.frame(growth)

growth.obs <- drop_na(growth_data, AGE_DAYS, MASS, MASS_MALE, TEMPERATURE) |> 
  mutate(Pred =predict(model1a.p2, re.form=NA, type ='response'), 
         Resid =residuals(model1a.p2, type ='response'), 
         Fit =Pred+Resid) 
