...
```{r}
sample_size_yolksac <- yolk_df_all |> 
  group_by(TEMPERATURE, MALE, FEMALE, POPULATION, CLUTCH_NUMBER) |> 
  summarise(n =n(), .groups = "drop"); sample_size_yolksac

write.csv(sample_size_yolksac, "../../sample_size_yolksac.csv")
```
...
yolk.emmeans.df <- model1b |> emmeans(~TEMPERATURE, type ="response") |> as.data.frame()
save(yolk.emmeans.df, file="Figure_files/yolk.emmeans.RData")
```
...
yolksize.plot <- ggplot(data = yolk.df, aes(x=MASS_FEMALE, y=response)) + 
  ...
  geom_pointrange(data = yolk.obs.summarize, aes(x =mean.mass.female, 
                                                  y =mean.yolksize, 
                                                  ymin =mean.yolksize -sd.yolksize, 
                                                  ymax =mean.yolksize +sd.yolksize, 
                                                  color = TEMPERATURE))  +  
  scale_color_manual(values = c("#69d7d8","#ff9c56", "#903146")) + 
...
#save(yolk.obs.summarize, file="Figure_files/yolk.obs.summarize.RData")
#save(yolk.obs.summarize2, file="Figure_files/yolk.obs.summarize2.RData")
```
